...
def main(args):
    ...
    mastered_samples_file = os.path.join(
        StatsLogger.get_log_path(config.stats_logger), "mastered_samples.txt"
    )

    train_dataloader = FilteredStatefulDataLoader(
        ...
        mastered_samples_file=mastered_samples_file,
    )
    ...
    workflow = VisionRLVRWorkflowWithQid(
    ...
    eval_workflow = VisionRLVRWorkflowWithQid(
    ...
    for global_step in range(start_step, max_steps):
        ...
        if actor.is_data_parallel_head():
            ...
            qids = batch.get("qid", None)
            if rewards is not None and qids is not None:
                ...
                if isinstance(qids, torch.Tensor):
                    qid_values = qids.cpu().tolist()
                else:
                    qid_values = qids

                for qid_val, reward_val in zip(qid_values, reward_values):
                    if reward_val >= 1.0:
                        train_dataloader.add_mastered_sample(qid_val)
