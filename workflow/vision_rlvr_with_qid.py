...
from .vision_rlvr import VisionRLVRWorkflow


def qid_to_int(qid):
    qid_str = str(qid)
    hash_val = int(hashlib.md5(qid_str.encode()).hexdigest()[:16], 16)
    return hash_val % (2 ** 31)


class VisionRLVRWorkflowWithQid(VisionRLVRWorkflow):
    async def arun_episode(self, engine, data):
        ...
        for i, resp in enumerate(resps):
            ...
            res = dict(
                ...
                qid=torch.tensor([qid_to_int(data.get("id"))], dtype=torch.int32),
            )
